class BillingPage {
    ...
    get startNewBillBtn() { return cy.contains("button", /Start a New Bill/i); }

    // Scoped container for the billing form using Check Eligibility as anchor
    get billingModal() {
        return cy.contains("button:visible", /CHECK ELIGIBILITY/i).closest(".MuiBox-root").parent();
    }
    ...
    initiateNewBill() {
        ...
        cy.get("body").then(($body) => {
            if ($body.find(':contains("Please fill in: clinicalNoteId")').length > 0) {
    ...
    fillBasicBillingInfo(dataCenter, payee, billTo, asType) {
        ...
        cy.get("@activeBillingForm").within(() => {
            cy.contains(/Data Center/i).parent().find('[role="combobox"]').first().click({ force: true });
        });
        cy.get('ul[role="listbox"]:visible li[role="option"]').first().click({ force: true });

        // 2. Select Payee
        cy.get("@activeBillingForm").within(() => {
            cy.contains(/Payee/i).parent().find('[role="combobox"]').first().click({ force: true });
        });
        cy.get('ul[role="listbox"]:visible li[role="option"]').first().click({ force: true });

        // 3. Select Bill To (e.g., MSP, ICBC)
        cy.get("@activeBillingForm").within(() => {
            cy.contains(/Bill to/i).parent().find('[role="combobox"]').first().click({ force: true });
        });
    ...
    fillFFSDetails() {
        cy.get("@activeBillingForm").within(() => {
            ...
            cy.get('button').then(($btns) => {
                const moreBtn = $btns.filter((i, el) => el.innerText.includes("MORE BILLING DETAILS"));
            ...
            fields.forEach(f => {
                cy.contains(f.label).parent().find("input").clear({ force: true }).type(f.value, { force: true });
            });
        });
    }
    ...
    fillINFields(address, postalCode, institutionNo) {
        cy.get("@activeBillingForm").within(() => {
            // Fill Address
            cy.contains(/Address/i).parent().find('textarea:visible').should('be.visible')
                .clear({ force: true }).type(address, { force: true });

            // Fill Postal Code
            cy.contains(/Postal Code/i).parent().find('input').should('be.visible')
                .clear({ force: true }).type(postalCode, { force: true });
    ...
    addServiceRow(diag, service, units = "1") {
        ...
        cy.contains('div', /Diag Code/i).closest('.MuiBox-root').as('diagCodeSection');
        ...
        cy.get("@diagCodeSection").within(() => {
            ...
            cy.get("table tbody tr").first().within(() => {
                cy.get("td").eq(3).find("input").clear({ force: true }).type(units, { force: true });
    ...
    saveAndCloseBill(note) {
        ...
        cy.get('body').then(($body) => {
            if ($body.find('#swal2-title:contains("Age-based Code Update")').length > 0) {
    ...
    closeDrawerAndVerifyReturn() {
        ...
        cy.contains('button', /NEW PROGRESS NOTE/i, { timeout: 30000 })
            .should('be.visible');
